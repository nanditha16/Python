...
#   The function whose ID is at the top of the stack is the current 
...
#  started at the beginning of timestamp 3, and "1:end:2" means a 
#  function call with function ID 1 ended at the end of timestamp 2. 
...
# 1. Use a stack to track which function is currently executing.
# 2. Track time between logs to compute how long each function runs exclusively.
# 3. On "start":
#     Pause the current function (if any) and record its time.
#     Push the new function onto the stack.
# 4. On "end":
#     Pop the function from the stack.
#     Add the time it ran (including the end timestamp).
#     Resume the previous function (if any) from the next timestamp.

# Edge Cases Handled
#     Recursive calls (same function ID multiple times)
#     Multiple nested calls
#     Empty or minimal logs
#     Functions starting and ending at same timestamp
...
class Solution:
    def exclusiveTime(self, n: int, logs: List[str]) -> List[int]:
        # Initialize result array to store exclusive time for each function
        result = [0] * n
        
        # Stack to simulate the function call stack
        stack = []
        
        # Variable to track the previous timestamp
        prev_time = 0

        # Iterate through each log entry
        for log in logs:
            # Parse the log into function ID, type (start/end), and timestamp
            fn_id_str, typ, time_str = log.split(":")
            fn_id = int(fn_id_str)
            time = int(time_str)

            if typ == "start":
                # If another function is currently running, update its time
                if stack:
                    result[stack[-1]] += time - prev_time
                # Push the new function onto the stack
                stack.append(fn_id)
                # Update previous time to current start time
                prev_time = time
            else:  # typ == "end"
                # Pop the function from the stack and update its time
                result[stack.pop()] += time - prev_time + 1
                # Update previous time to the next time unit after end
                prev_time = time + 1
