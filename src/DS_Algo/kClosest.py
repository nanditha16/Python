...
# We use x² + y² instead of √(x² + y²) because the square root 
# is monotonic—it preserves order—so we can skip computing it for performance.

# Why heapq.nsmallest?
# It’s efficient for small k compared to sorting the entire list.
#  Time complexity is approximately O(n log k).
...
class Solution:
    def kClosest(self, points: List[List[int]], k: int) -> List[List[int]]:
        # Use a min-heap based on squared Euclidean distance
